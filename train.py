...
def train(logger, config, consts, paths):
    ...
    if config.resume_iteration == 0:
        ...
        pass
    scheduler = StepLR(
        optim, step_size=config.lr_decay_step, gamma=config.lr_decay_rate
    )
    early_stopper = EarlyStop(
        patience=config.patience, goal=config.goal, delta=config.delta
    )
    stop = "best"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "logger", choices=["wandb", "tensorboard", "none"], nargs="?", default="wandb"
    )
